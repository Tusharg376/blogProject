...
const login = async function (req, res) {
    try {
        ...
        if (!findAuthor) return res.status(400).send({ status: false, msg: "credentials don't match" })
...
const getBlogs = async function (req, res) {
    try {
        ...
        if(queryParams.authorId) if(!mongoose.isValidObjectId(queryParams.authorId)) return res.status(400).send({status:false,msg:"AuthorId is invalid"})
        ...
        if (blogs.length == 0) return res.status(400).send({ status: false, Status: false, msg: "Blogs doesn't exist" })
...
const deleteBlog = async function (req, res) {
    try {
        ...
        if (!deleteDoc) return res.status(400).send({ status: false, msg: "Document not found" })
...
const deleteByQuery = async function (req, res) {
    try {
        ...
        if (!deleteBlog) return res.status(400).send({ status: false, msg: "docs not found" });
